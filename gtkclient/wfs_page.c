...
void fill_wfs_page(GtkWidget *vbox)
{
	...
	label = gtk_label_new("#Att:");
        gtk_box_pack_start(GTK_BOX(hbox), label , TRUE, TRUE, 0);
        gtk_widget_set_usize (label, LABAO_WIDTH/11, LABAO_HEIGHT);
	...
	tries_entry = gtk_entry_new ();
        ...
        gtk_widget_set_usize (tries_entry, LABAO_WIDTH/8, LABAO_HEIGHT);
        gtk_widget_show(tries_entry);

        button = gtk_button_new_with_label ("ALIGN LAB");
	gtk_signal_connect (GTK_OBJECT (button), "clicked",
        ...
        button = gtk_button_new_with_label ("ALIGN SCOPE");
	gtk_signal_connect (GTK_OBJECT (button), "clicked",
	...
	num_entry = gtk_entry_new ();
        gtk_entry_set_text (GTK_ENTRY (num_entry),"1000");
        gtk_box_pack_start(GTK_BOX(hbox), num_entry, TRUE, TRUE, 0);
        gtk_widget_set_usize (num_entry, LABAO_WIDTH/8, LABAO_HEIGHT);
